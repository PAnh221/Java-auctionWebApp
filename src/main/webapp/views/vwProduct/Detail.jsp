          ...
          <c:choose>
            ...
            <c:otherwise>
              <c:forEach items="${bidHistory}" var="h">
                <tr>
                  <td>${h.time}</td>
                  <td>${authUser.userID == product.sellerID? h.userName : h.encodeUserName}<a href="${pageContext.request.contextPath}/Rating/Detail?Username=${h.userName}"> (${h.reputationOfBidder}%)</a></td>
                  <td>
